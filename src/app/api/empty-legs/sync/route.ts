...
/* ✅ ALLOW BOTH INTERNAL + EXTERNAL CALLS */
function isAuthorized(request: NextRequest) {
  const authHeader = request.headers.get("authorization");

  // External (Postman, curl)
  if (authHeader === `Bearer ${CRON_SECRET}`) return true;

  // Internal (server-to-server call, no auth header)
  const host = request.headers.get("host");
  const origin = request.headers.get("origin");

  if (!authHeader && origin && host && origin.includes(host)) {
    return true;
  }
  ...
  try {
    ...
    while (hasMore) {
      const res = await fetch(`${PEXJET_API_URL}?page=${page}&limit=${limit}`, {
        headers: { Authorization: `Bearer ${PEXJET_TOKEN}` },
      });
      ...
      if (!meta || page >= meta.totalPages) hasMore = false;
      else page++;
    }
