  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
      <div className="space-y-2">
        ...
        <Input id="email" type="email" placeholder={content.auth.login.emailPlaceholder} disabled={isLoading} {...register("email")} />
        <FieldError>{errors.email?.message}</FieldError>
      </div>

      <div className="space-y-2">
        <Label htmlFor="password">{content.auth.login.passwordLabel}</Label>
        <PasswordInput id="password" placeholder={content.auth.login.passwordPlaceholder} disabled={isLoading} {...register("password")} />
      ...
      <Link href="/forgot-password" className="mt-6 block text-center text-muted-foreground text-sm hover:text-foreground">
