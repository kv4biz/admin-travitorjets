...
import { useRaisedShadow } from "@/components/ui/useRaisedShadow";
import { cn } from "@/lib/utils";

const ReorderList: React.FC<ReorderListProps> = ({ className, itemClassName, withDragHandle = false, onReorderFinish, ...props }) => {
  const [items, setItems] = useState<React.ReactElement[]>(
    React.Children.toArray(props.children).filter((child) => React.isValidElement(child)) as React.ReactElement[],
  );
  ...
  return (
    ...
    >
      {items.map((item, index) => (
        <ReorderListItem key={item?.key || index} item={item} withDragHandle={withDragHandle} className={itemClassName} />
      ))}
    </Reorder.Group>
  );
};
...
const ReorderListItem: React.FC<{
  ...
  return (
    <Reorder.Item
      ...
      id={item?.key ?? ""}
      value={item}
      className={cn("bg-background list-none p-0! m-0!", !withDragHandle ? "cursor-grab" : "", className)}
      style={{ boxShadow, y }}
